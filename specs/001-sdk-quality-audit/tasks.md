...
- [x] T101 **DONE this PR.** Added `INTENTIONAL_DEVIATIONS` registry to `scripts/audit-consistency.ts` (analogous to `INTENTIONALLY_SKIPPED` in `verify-endpoints.ts`). Each entry is `{ resource, method, attribute, reason }`; the divergence detector filters matching occurrences before counting variants, and if filtering drops the variant count below 2 the cluster is suppressed entirely. Seeded with `dayOffTypes.list / param_shape` because the upstream OpenAPI spec declares no `requestBody` for `/dayOffTypes.list`. Closes the consistency finding `93f5f2e8211ae060`. PATCH bump (audit-internal, no SDK public-API impact).
...
- [x] T104 **DONE this PR — outcome was different from the original plan.** On close inspection, all 4 supposed "bare throw" sites turned out to be false positives in my own audit, not real principle V violations:
  * `src/paginator.ts:45` and `src/client.ts:421` — conditional throws that propagate `signal.reason` if it's an Error, else construct a Web Standard `DOMException("AbortError")`. Abort-signal handling; principle V is about HTTP failure paths, not signal cancellation.
  * `src/client.ts:454` and `src/client.ts:497` — re-throws (`throw error`, `throw refreshError`) that propagate an already-typed error from a catch binding. The class identity is preserved through the re-throw; no new untyped error is constructed.

  The right fix was to make the audit detector smarter, NOT to add a new public error class. Updated `scripts/audit-lib/ast.ts::getThrowSites` to classify each throw as `"new" | "rethrow" | "conditional" | "other"` and capture all reachable constructor names for conditionals. Updated `scripts/audit-compliance.ts::findBareThrows` to: (a) never flag re-throws, (b) accept `DOMException` as a valid throw target (Web Standard), (c) only flag conditional throws if ANY branch constructs a non-acceptable type. Closes findings `315f1b37a6a17009`, `317f220ee8378e37`, `a1efe1a403b7d831`, `b0f2a062e828281a`. Zero src/ touches; no new error class introduced. PATCH bump (audit-internal).

- [x] T105 **DONE this PR.** Updated the README Resources table to reflect actual SDK exports: removed `dealPhases.duplicate` (intentionally unimplemented per upstream 404), added the 13 missing payment-related methods on `incomingCreditNotes`/`incomingInvoices`/`receipts` and `calls.delete`. Closes 14 docs findings. Also fixed audit-docs.ts classifier so future docs drift is correctly tagged as `trivial` + `in-pr` (single file, no API change, no behavior change per FR-008) rather than spawning new remediation tasks.

- [x] T106 **DONE this PR.** Chose option (b): the 5 untagged CHANGELOG entries (`[0.0.1]` through `[0.3.0]`) were demoted from `## [X.Y.Z]` to `### [X.Y.Z]` under a new `## Pre-tagging early development (v0.0.1 – v0.3.0)` parent heading with a note explaining the historical context. Closes 5 changelog findings. Also fixed audit-changelog.ts classifier so `missing-git-tag` findings are correctly tagged as `trivial` + `in-pr` (single file edit OR single git tag command) rather than spawning remediation tasks.
